...
class TestS3DownloadFile:
    @patch("apps.documents.services.uploaded_files.s3._ensure_bucket_exists")
    @patch("apps.documents.services.uploaded_files.s3._get_client", autospec=True)
    def test_download__success(self, mock_get_client, mock_ensure_bucket_exists):
        mock = Mock()
        mock_get_client.return_value = mock
        mock.get_object.return_value = {"Body": Mock()}
        mock_ensure_bucket_exists.return_value = True
        assert s3_download_file("s3_key")

    @patch("apps.documents.services.uploaded_files.s3._ensure_bucket_exists")
    @patch("apps.documents.services.uploaded_files.s3._get_client")
    def test_download_fails_if_bucket_does_not_exist(
        ...
        with pytest.raises(ClientError):
            s3_download_file("s3_key")

    @patch("apps.documents.services.uploaded_files.s3._ensure_bucket_exists")
    @patch("apps.documents.services.uploaded_files.s3._get_client", autospec=True)
    def test_download__fails_on_file_download(
        self, mock_get_client, mock_ensure_bucket_exists
    ):
        mock = Mock()
        mock_get_client.return_value = mock
